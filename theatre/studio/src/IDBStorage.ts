...
export const createStore = (name: string) => {
  ...
  return {
    set: (key: string, value: unknown) => idb.set(key, value, customStore),
    get: <T = unknown>(key: string) => idb.get<T>(key, customStore),
    del: (key: string) => idb.del(key, customStore),
    keys: <T extends IDBValidKey>() => idb.keys<T>(customStore),
    entries: <KeyType extends IDBValidKey, ValueType = unknown>() =>
      idb.entries<KeyType, ValueType>(customStore),
    values: <T = unknown>() => idb.values<T>(customStore),
  }
}
